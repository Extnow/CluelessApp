...
function OverlayMenu(props) {
  const { playerNames, handleNewGameSubmit, setIsPlaying } = props;

  const [newListOfPlayers, setNewPlayersList] = useState(playerNames);
  const [playground, setPlayground] = useState('house');
  ...
  useEffect(() => {
    setNewPlayersList(state => {
      ...
      if (state.length < amountOfPlayers)
        return state.concat(Array.from({ length: amountOfPlayers - state.length }, () => ''));
  ...
  return (
    <div
      id="overlay-menu"
      className="z-50 w-full h-screen top-0 fixed"
      style={{ backgroundColor: 'rgba(134,173,146, 0.7)' }}
    >
      <div className="max-w-screen-sm mx-auto h-screen flex justify-center items-center px-2">
        <div className="w-full text-center bg-gray-400 rounded border border-black p-4">
          <div className="mb-2">New Game</div>
          <div className="text-base md:text-2xl">
            <div className="mb-2">
              <span className="">Choose playground</span>
              <div className="flex gap-2 mt-2 justify-center">
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    playground === 'house' && 'bg-purple-400 text-white'
                  }`}
                  onClick={() => setPlayground('house')}
                >
                  Особняк
                </button>
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    playground === 'seafront' && 'bg-purple-400 text-white'
                  }`}
                  onClick={() => setPlayground('seafront')}
                >
                  Набережная
                </button>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2 justify-center">
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    amountOfPlayers === 3 ? 'bg-purple-400 text-white' : ''
                  }`}
                ...
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    amountOfPlayers === 4 ? 'bg-purple-400 text-white' : ''
                  }`}
                ...
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    amountOfPlayers === 5 ? 'bg-purple-400 text-white' : ''
                  }`}
                ...
                <button
                  className={`text-base rounded px-2 md:text-xl border-2 border-purple-400 focus:outline-none ${
                    amountOfPlayers === 6 ? 'bg-purple-400 text-white' : ''
                  }`}
            ...
            <div className="grid grid-cols-3 md:grid-cols-2 gap-4 mt-4 w-full">
              {amountOfPlayers > 0 &&
                Array.from({ length: amountOfPlayers - 1 }, () => null).map((value, i) => (
